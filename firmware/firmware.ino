...
uint8_t color;
const uint16_t primaryColors[] = {
  matrix.Color(255, 255, 255), matrix.Color(160, 0, 0), matrix.Color(0, 148, 0), matrix.Color(0, 0, 160),
  matrix.Color(255, 255, 0), matrix.Color(0, 255, 255), matrix.Color(255, 0, 255), matrix.Color(255, 255, 255),
};

const uint16_t adaColors[] = {
  matrix.Color(255, 0, 0),   //A red
  matrix.Color(255, 200, 0), //D orange
  matrix.Color(200, 255, 0), //A yellowish
  matrix.Color(0, 255, 0),   //F green
  matrix.Color(0, 255, 225), //R blue
  matrix.Color(160, 0, 255), //U purple
...
void strips() {
  ...
  for (uint16_t x = 0; x < 8; x++) {
    for (uint16_t y = 0; y < 8; y++) {
      matrix.drawPixel(x, y, primaryColors[y]);
    }
  }
  matrix.show();
  delay(500);
...
void mondrian1() {
  ...
  matrix.drawPixel(7, 7, matrix.Color(255, 255, 0));
  matrix.show();
  delay(500);
}
...
void randPattern() {
  ...
  for (uint16_t x = 0; x < 8; x++) {
    for (uint16_t y = 0; y < 8; y++) {
      c = random(0, 10);
      matrix.drawPixel(x, y, adaColors[c]);
    }
  }
...
void loop(void)
{
  ...
  // matrix.fillScreen(primaryColors[color]);
  // matrix.show();
  // delay(500);
  ...
  if (strcmp(ble.buffer, "strips") == 0) {
    color = 0;
  } else if (strcmp(ble.buffer, "mondrian1") == 0) {
    color = 1;
  } else if (strcmp(ble.buffer, "mondrian2") == 0) {
    color = 2;
  } else if (strcmp(ble.buffer, "random") == 0) {
    color = 3;
  }
